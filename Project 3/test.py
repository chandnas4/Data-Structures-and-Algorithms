...
class TestProject3(unittest.TestCase):
    ...
    def test_comprehensive(self):
        ...
        #vals = [s for s in string.ascii_letters]
        #random.shuffle(vals)
        ...
        for val in vals:
            ...
            if tree.root:
                ...
                print()
                print("My tree")
                print([x.value for x in tree.inorder(tree.root)])
                print([x.is_red for x in tree.inorder(tree.root)])
                print("Solution:")
                print((copy))
                assert [x.value for x in tree.inorder(tree.root)] == copy
...
"""
            if node_2:
                if node_2 is self.root:
                    self.root = None

                if val< node_2.value:
                    node.left = BST_remove(node_2.left, val)

                elif (val > node_2.value):
                    node_2.right = BST_remove(node_2.right, val)

                else:
                    if node_2.left is None and node_2.right:
                        temp = node_2.right
                        node_2 = None
                        return temp

                    elif node.right is None and node_2.left:
                        temp = node_2.left
                        node_2 = None
                temp = self.min(node_2.right)

                # Copy the inorder successor's content to this node
                node_2.value = temp.value

                # Delete the inorder successor
                node_2.right = BST_remove(node_2.right, temp.value)
                """
